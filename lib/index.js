...
class Config {
    constructor() {
        ...
        coc_nvim.workspace.onDidChangeConfiguration((event) => this.onConfigChange(event));
        this.cfg = coc_nvim.workspace.getConfiguration(this.rootSection);
        if (!this.cfg) {
    ...
    async onConfigChange(event) {
        ...
        const prompt = await coc_nvim.workspace.showPrompt(`${msg}. Reload now?`);
        if (prompt) {
            await coc_nvim.commands.executeCommand(`workbench.action.reloadWindow`);
        }
    }
    async update(section, value, isUser) {
    ...
    get addNewBuiltins() {
        return this.cfg.get('extension.actions.schema.addNewBuiltins', 'ask');
    }
    get removeOldBuiltins() {
        return this.cfg.get('extension.actions.schema.removeOldBuiltins', 'ask');
    }
    get defaultAssociations() {
        var _a, _b;
        return (_b = (_a = this.cfg.inspect('schema.associations')) === null || _a === void 0 ? void 0 : _a.defaultValue) !== null && _b !== void 0 ? _b : {};
    }
    get currentAssociations() {
        var _a;
        return (_a = this.cfg.get('schema.associations')) !== null && _a !== void 0 ? _a : {};
    }
    get debug() {
        return this.cfg.get('debug');
    }
    async setAssociations(value, isUser) {
        await this.update('schema.associations', value, isUser);
    }
    async setAddNewBuiltins(value, isUser) {
        await this.update('extension.actions.schema.addNewBuiltins', value, isUser);
    }
    async setRemoveOldBuiltins(value, isUser) {
        await this.update('extension.actions.schema.removeOldBuiltins', value, isUser);
    }
}
...
function createClient(p) {
    const serverOpts = {
        run: { module: p, transport: coc_nvim.TransportKind.ipc },
        debug: { module: p, transport: coc_nvim.TransportKind.ipc },
    };
    const outputChannel = coc_nvim.workspace.createOutputChannel('Taplo Language Server Trace');
    const clientOpts = {
        documentSelector: [
        ...
        initializationOptions: {
            configuration: coc_nvim.workspace.getConfiguration('coc-toml'),
        },
        synchronize: {
            configurationSection: 'coc-toml',
            fileEvents: [
                coc_nvim.workspace.createFileSystemWatcher('**/.toml'),
                coc_nvim.workspace.createFileSystemWatcher('**/Cargo.lock'),
            ],
        },
        outputChannel,
    };
    // Create client for toml
    const client = new coc_nvim.LanguageClient('toml', 'toml language server', serverOpts, clientOpts);
...
function isAbsolutePath(p) {
    return (path__default['default'].resolve(p) === path__default['default'].normalize(p).replace(RegExp(path__default['default'].sep + '$'), ''));
}
...
var sideChannel = function getSideChannel() {
	...
	var channel = {
		assert: function (key) {
			if (!channel.has(key)) {
				throw new $TypeError$5('Side channel does not contain ' + objectInspect(key));
			}
		},
...
var hasToStringTag$6 = hasSymbols$6 && typeof Symbol.toStringTag === 'symbol';
...
var hasToStringTag$7 = hasSymbols$7 && typeof Symbol.toStringTag === 'symbol';
...
class Ctx {
    constructor(extCtx, config) {
        ...
        this.statusBar = coc_nvim.workspace.createStatusBarItem(10);
    ...
    registerCommand(name, factory) {
        ...
        const d = coc_nvim.commands.registerCommand(fullName, cmd);
        this.extCtx.subscriptions.push(d);
    }
    serverPath() {
        return this.extCtx.asAbsolutePath(path__default['default'].join('lib', 'server.js'));
    }
    async startServer() {
        const client = createClient(this.serverPath());
        this.extCtx.subscriptions.push(coc_nvim.services.registLanguageClient(client));
        if (this.config.showNotification) {
            // show loading status.
            const statusItem = coc_nvim.workspace.createStatusBarItem(0, { progress: true });
        ...
        client.onNotification(WatchConfigFile.METHOD, this.watchServerTaploConfigFile);
    ...
    showMessage(params) {
        ...
        coc_nvim.workspace.showMessage(params.message, _trans(params.kind));
    }
    watchConfigFile() {
        var _a, _b;
        (_a = this.taploConfigWatcher) === null || _a === void 0 ? void 0 : _a.close();
        this.taploConfigWatcher = undefined;
        const cfgPath = this.config.taploConfig;
        if (typeof cfgPath === 'string' && cfgPath.length > 0) {
            let p = cfgPath;
            if (!isAbsolutePath(p)) {
                const wsPath = (_b = coc_nvim.workspace.workspaceFolders[0]) === null || _b === void 0 ? void 0 : _b.uri;
                if (typeof wsPath !== 'undefined') {
                    return;
                }
                p = path__default['default'].join(wsPath, p);
            }
            this.taploConfigWatcher = fs__default['default'].watch(p);
            this.taploConfigWatcher.on('change', () => {
                this.client.sendNotification(ConfigFileChanged.METHOD);
            });
        }
    }
    watchServerTaploConfigFile(params) {
        var _a;
        (_a = this.serverTaploConfigWatcher) === null || _a === void 0 ? void 0 : _a.close();
        this.serverTaploConfigWatcher = undefined;
        const ws = coc_nvim.workspace.workspaceFolder.uri;
        if (typeof ws === 'undefined') {
            return;
        }
        this.serverTaploConfigWatcher = fs__default['default'].watch(params.configPath, () => {
            this.client.sendNotification(ConfigFileChanged.METHOD);
        });
    }
    // cache schema ============================================================
    get schemaPath() {
        return path__default['default'].join(this.extCtx.storagePath, 'schemas.json');
    }
    async _schemaData() {
        let schemas = {};
        try {
            schemas = JSON.parse(await coc_nvim.workspace.readFile(this.schemaPath));
        }
        catch (e) {
            // Doesn't yet exist.
        }
    ...
    async cacheSchema(params) {
        const schemas = await this._schemaData();
        schemas[params.schemaUri] = params.schemaJson;
        const doc = coc_nvim.workspace.getDocument(this.schemaPath);
        const _range = allRange(doc);
        const _edit = TextEdit.replace(_range, JSON.stringify(schemas));
        await coc_nvim.workspace.getDocument(this.schemaPath).applyEdits([_edit]);
    }
    async getCachedSchema(params) {
    ...
    //   throw 'unreachable';
    // }
    async updateAssociations(params) {
        if ((this === null || this === void 0 ? void 0 : this.config) === undefined) {
            console.error('No context!');
            return;
        }
        const updateNew = this.config.addNewBuiltins;
        const removeOld = this.config.removeOldBuiltins;
        if (updateNew === 'never' && removeOld === 'never') {
            return;
        }
        const defaultAssociations = this.config.defaultAssociations;
        let currentAssociations = this.config.currentAssociations;
        ...
        if (updateNew !== 'never') {
            ...
            if (needUpdate) {
                // update
                const msg = 'There are new built-in schemas available. Update the associations?';
                if (updateNew === 'ask') {
                    const action = await coc_nvim.workspace.showQuickpick(['Update', 'Never Update', 'Always Update'], msg);
                    switch (action) {
                        case 0:
        ...
        if (removeOld !== 'never') {
            ...
            if (needRemove) {
                if (removeOld === 'ask') {
                    const msg = 'There are deprecated built-in schemas in associations. Remove them?';
                    const action = await coc_nvim.workspace.showQuickpick(['Remove', 'Never Remove', 'Always Remove'], msg);
                    switch (action) {
                        case 0:
...
function allRange(doc) {
    return Range.create(main$2.Position.create(0, 0), main$2.Position.create(doc.lineCount - 1, doc.getline(doc.lineCount - 1).length - 1));
}

function tomlToJson(ctx) {
    return async () => {
        var _a, _b;
        const doc = await coc_nvim.workspace.document;
        const mode = await coc_nvim.workspace.nvim.call('visualmode');
        let range;
        if (mode) {
            range = await coc_nvim.workspace.getSelectedRange(mode, doc);
        }
        else {
            // Select all range
            range = Range.create(main$2.Position.create(0, 0), main$2.Position.create(doc.lineCount - 1, doc.getline(doc.lineCount - 1).length - 1));
        }
        const text = doc.textDocument.getText(range);
        const params = {
            text: text,
        };
        const res = await ctx.client.sendRequest(TomlToJson.METHOD, params);
        const errLines = [];
        if ((_b = (_a = res.errors) === null || _a === void 0 ? void 0 : _a.length) !== null && _b !== void 0 ? _b : 0 !== 0) {
            ...
            await coc_nvim.workspace.showMessage('Copying has failed!', 'error');
            const show = await coc_nvim.workspace.showQuickpick(['Yes', 'No'], 'Show Details');
            if (show === 0) {
                await coc_nvim.workspace.echoLines(errLines);
            }
            return;
        }
        if (!res.text) {
            await coc_nvim.workspace.showMessage(`The response shouldn't be empty, but it is.`);
            await coc_nvim.workspace.showMessage('Copying has failed!', 'error');
            const show = await coc_nvim.workspace.showQuickpick(['Yes', 'No'], 'Show Details');
            if (show === 0) {
                await coc_nvim.workspace.echoLines(errLines);
        ...
        await coc_nvim.workspace.nvim.command('tabnew').then(async () => {
            const buf = await coc_nvim.workspace.nvim.buffer;
            buf.setLines(res.text.split('\n'), { start: 0, end: -1 });
        });
        await coc_nvim.workspace.showMessage('JSON has been generated!');
    };
}
...
function syntaxTree(ctx) {
    return async () => {
        ...
        const res = await ctx.client.sendRequest(SyntaxTree.METHOD, params);
        await coc_nvim.workspace.nvim.command('tabnew').then(async () => {
            const buf = await coc_nvim.workspace.nvim.buffer;
            buf.setLines(res.text.split('\n'), { start: 0, end: -1 });
        });
...
async function activate(context) {
    ...
    if (!config.enabled) {
        ...
        coc_nvim.workspace.showMessage('activate stopped because of: configs.enabled is false');
    }
    ctx.registerCommand('tomlToJson', tomlToJson);
    ctx.registerCommand('syntaxTree', syntaxTree);
    ctx.registerCommand('reload', (ctx) => {
        return async () => {
            coc_nvim.workspace.showMessage(`Reloading taplo...`);
            ...
            coc_nvim.workspace.showMessage(`Reloaded taplo`);
        };
    });
